...
- **内存泄露（Memory-Leak）检测**：结合 openGauss MemoryContext 生命周期、错误跳转和长期后台线程路径检查泄露风险
...
```
bug-detection-audit <代码目录> [doublefree|memoryleak|all] [domain=logical-decoding]
```

- `代码目录路径`：要审计的代码目录
- `审计类型`（默认 `all`）：`doublefree` / `memoryleak` / `all`
...
│   ├── memory-leak-audit.md          # 内存泄露审计清单 (L1-L10)
